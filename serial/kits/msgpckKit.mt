    ...
    def extHandler (extNr :Integer, bufferIn :Bytes, consumer :Sink) :Vow[Void] {
      ...
      switch (extNr) {
        ...
        match ==13 {
          ...
          var import_pos :Msgpck["uint"]
          var rdr_pos    :Msgpck["uint"]
          var rdrBase    :Msgpck["binary"]
          object my_newRemotePromiseDesc_irr_sink {
            to run(items) :Vow[Void] {
              import_pos := items[0]
              rdr_pos    := items[1]
              rdrBase    := items[2]
          ...
          def irr_buff := makeBufferSrc(parserSrc, 3)
          return irr_buff <- run(my_newRemotePromiseDesc_iir_sink)
        ...
        match ==14 {
          ...
          var nonce      :Msgpck["binary"]
          var vine       :Any    # a vine can be anything.
          object my_newPromise3Desc_sphnv_sink {
            to run(items) :Vow[Void] {
              searchPath := items[0]
              hostId     := items[1]
              nonce      := items[2]
          ...
          def sphnv_buff := makeBufferSrc(parserSrc, 4)
          return sphnv_buff <- run(my_newPromise3Desc_sphnv_sink)
        }
        match ==15 {
          # RemoteDeliver
          var recipiant :Any
